/*
  Program: lab9_data-race1_prod+cons_synced.c (Varianta sincronizată !)
...
void consumer_work()
{
    ...
    for(i = 0; i < nr_texte; i++)
    {
        /* Ideea de sincronizare: așteptăm până când producătorul va scrie al i-lea text (adică până când nu mai "vedem" zerouri la acele adrese...)
           De fapt, conform celor explicate la acest exercițiu în pagina cu laboratorul #9, este suficient să așteptăm până "vedem" non-zero doar la
           penultima adresă, dintre cele 10 adrese consecutive folosite pentru a stoca textul al i-lea.
		*/
		
        /* varianta 1: așteptare ocupată */
        //do{ } while( *(map_addr + (i+1)*dim_texte - 2) == 0); // I am "spinning" on the CPU, testing repeatedly for zero...

        /* varianta 2: așteptare blocantă */
        do{ sleep(1); } while( *(map_addr + (i+1)*dim_texte - 2) == 0); // I will sleep for 1 second in the waiting state, then I will become ready, then running and test again for zero...

        /* varianta 3: the minimal non-spinning waiting */
        // do{ sleep(0); } while( *(map_addr + (i+1)*dim_texte - 2) == 0); // I am going to the ready state (i.e., I preempt myself, but without an effective sleep), then running and test again for zero...
